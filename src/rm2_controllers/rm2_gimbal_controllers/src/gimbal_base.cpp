...
controller_interface::CallbackReturn GimbalControllerBase::on_activate(
  ...
  if (pid_debug_publisher_) {
    pid_debug_publisher_->reset(last_publish_time_);
  }
...
GimbalControllerBase::update(const rclcpp::Time &time,
  ...
  if (pid_debug_publisher_) {
    pid_debug_publisher_->publish("~/debug/yaw_pid", gimbal_frame_id_, time,
                                  yaw_controller_.getPositionError(),
                                  yaw_controller_.getOuterLoopVelocityReference(),
                                  yaw_controller_.getInnerLoopVelocityError());
    pid_debug_publisher_->publish(
        "~/debug/pitch_pid", gimbal_frame_id_, time,
