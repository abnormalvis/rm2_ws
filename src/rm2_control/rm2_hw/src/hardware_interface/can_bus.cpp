...
namespace rm2_hw
{
CanBus::CanBus(rclcpp::Node::SharedPtr node,const std::string& bus_name, CanDataPtr data_ptr, int thread_priority)
  : bus_name_(bus_name), node_(node), data_ptr_(data_ptr)
{
  // Initialize device at can_device, false for no loop back.
  while (!socket_can_.open(node_, bus_name, [this](const auto& arg) { this->frameCallback(arg); }, thread_priority) && rclcpp::ok())
    rclcpp::sleep_for(std::chrono::milliseconds(500));

  RCLCPP_INFO(node_->get_logger(),"Successfully connected to %s.", bus_name.c_str());
...
void CanBus::read(rclcpp::Time time)
{
  ...
  for (const auto& frame_stamp : read_buffer_)
  {
    ...
    else if (frame.can_id == static_cast<unsigned int>(0x000))
    {
      if (data_ptr_.id2act_data_->find(frame.data[0]) != data_ptr_.id2act_data_->end())
      {
        ...
        {  // MIT Cheetah Motor
          ...
          // Low pass filter
          act_data.lp_filter->input(act_data.vel, node_->now().seconds());
          act_data.vel = act_data.lp_filter->output();
          continue;
        }
      }
    ...
    // }
    if (frame.can_id != 0x0)
      RCLCPP_ERROR_STREAM_ONCE(node_->get_logger(),"Can not find defined device, id: 0x" << std::hex << frame.can_id
                                                                  << " on bus: " << bus_name_);
  }
  read_buffer_.clear();
}
...
{
  std::lock_guard<std::mutex> guard(mutex_);
  CanFrameStamp can_frame_stamp{ .frame = frame, .stamp = node_->now() };
