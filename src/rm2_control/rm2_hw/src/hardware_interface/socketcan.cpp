...
bool SocketCAN::open(rclcpp::Node::SharedPtr node, const std::string& interface, boost::function<void(const can_frame& frame)> handler,
                     int thread_priority)
{
  node_ = node;
  ...
  if (sock_fd_ == -1)
  {
    RCLCPP_ERROR(node_->get_logger(), "Error: Unable to create a CAN socket");
    return false;
  }
  ...
  {
    RCLCPP_ERROR(node_->get_logger(), "Unable to select CAN interface %s: I/O control error", name);
  ...
  if (rc == -1)
  {
    RCLCPP_ERROR(node_->get_logger(), "Failed to bind socket to %s network interface", name);
    close();
    return false;
  }
...
void SocketCAN::write(can_frame* frame) const
{
  if (!isOpen())
  {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *node_->get_clock(), 5000, "Unable to write: Socket %s not open", interface_request_.ifr_name);
    return;
  }
  if (::write(sock_fd_, frame, sizeof(can_frame)) == -1)
    RCLCPP_DEBUG_THROTTLE(node_->get_logger(), *node_->get_clock(), 5000, "Unable to write: The %s tx buffer may be full", interface_request_.ifr_name);
}
...
bool SocketCAN::startReceiverThread(int thread_priority)
{
  ...
  if (rc != 0)
  {
    RCLCPP_ERROR(node_->get_logger(), "Unable to start receiver thread");
    return false;
  }
  RCLCPP_INFO(node_->get_logger(), "Successfully started receiver thread with ID %lu", receiver_thread_id_);
