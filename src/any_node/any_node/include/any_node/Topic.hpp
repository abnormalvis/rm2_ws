...
#else  /* ROS2_BUILD */
[[deprecated("Use the parameters_interface instead.")]] typename rclcpp::Publisher<msg>::SharedPtr advertise(
  ...
  auto topic{parameterInterface->get_parameter("publishers." + name + ".topic").as_string()};
  auto queueSize{parameterInterface->get_parameter("publishers." + name + ".queue_size").as_int()};
...
#else  /* ROS2_BUILD */
[[deprecated("Use the parameters_interface instead.")]] ThreadedPublisherPtr<msg> threadedAdvertise(
...
#else  /* ROS2_BUILD */
[[deprecated("Use the parameters_interface instead.")]] typename rclcpp::Subscription<M>::SharedPtr subscribe(
    rclcpp::Node& nh, const std::string& name, [[deprecated]] const std::string& /* defaultTopic */,
  ...
  auto topic = parameterInterface->get_parameter("subscribers." + name + ".topic").as_string();
  auto queueSize = parameterInterface->get_parameter("subscribers." + name + ".queue_size").as_int();
...
ThrottledSubscriberPtr<M, T> throttledSubscribe(double timeStep, ros::NodeHandle& nh, const std::string& name,
                                                ...
                                                const ros::TransportHints& transport_hints = ros::TransportHints()) {
#else  /* ROS2_BUILD */
[[deprecated("Use the parameters_interface instead.")]] ThrottledSubscriberPtr<M, T> throttledSubscribe(
  ...
  if (paramInterface->get_parameter("subscribers/" + name + "/deactivate").as_bool()) {
#endif                                                     /* ROS2_BUILD */
...
#else  /* ROS2_BUILD */
    return std::make_shared<ThrottledSubscriber<M, T>>(
        timeStep, nh, paramInterface->get_parameter("subscribers." + name + ".topic").as_string(),
        paramInterface->get_parameter("subscribers." + name + ".queue_size").as_int(), fp, obj);
#endif /* ROS2_BUILD */
  }
}
...
[[deprecated("Use the parameters_interface instead.")]] auto advertiseService(
    ...
    rclcpp::CallbackGroup::SharedPtr group = nullptr) {
  auto service = nh.get_node_parameters_interface()->get_parameter("servers." + name + ".service").as_string();
...
ros::ServiceClient serviceClient(ros::NodeHandle& nh, const std::string& name, const std::string& defaultService,
                                 const ros::M_string& header_values = ros::M_string()) {
#else  /* ROS2_BUILD */
[[deprecated("Use the parameters_interface instead.")]] auto serviceClient(rclcpp::Node& nh, const std::string& name,
  ...
  return nh.serviceClient<Service>(param<std::string>(nh, "clients/" + name + "/service", defaultService),
                                   param<bool>(nh, "clients/" + name + "/persistent", false), header_values);
#else  /* ROS2_BUILD */
  auto service = nh.get_node_parameters_interface()->get_parameter("clients." + name + ".service").as_string();
